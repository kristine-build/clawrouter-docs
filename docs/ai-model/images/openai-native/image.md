...
| mask | file | no | 附加影像，其完全透明区域（例如，alpha 为零的区域）指示image应編輯的位置。必須是有效的 PNG 檔案，小于 4MB，并且尺寸與原始image相同。 Format`binary`.. |
...
```
curl -X POST "https://clawrouter.com/v1/images/edits/" \  -F image="cmMtdXBsb2FkLTE2ODc4MzMzNDc3NTEtMjA=/31225951_59371037e9_small.png" \  -F prompt="A cute baby sea otter wearing a beret."
```..
