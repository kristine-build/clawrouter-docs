...
| messages | array<object> | yes | 对话消息列表 |
| temperature | number | no | 采样温度 Default`1` Range`0 <= value <= 2` |
| top_p | number | no | 核采样参数 Default`1` Range`0 <= value <= 1` |
| n | integer | no | 生成数量 Default`1` Range`1 <= value` |
| stream | boolean | no | 是否流式响应 Default`false` |
...
| seed | integer | no |  |
| reasoning_effort | string | no | 推理强度 (用于支持推理的模型) Value in`"low" \| "medium" \| "high"` |
